...
class Pr007{
	public static void main(String args[]) {
		...
		System.out.println("Текущее i= " + i);
		//Оператор break во вложенных циклах
		for(i=0; i<3; i++){
			System.out.println("Счетчик внешнего цилка: " + i);
			System.out.print("	счетчик внутреннего цилка: ");
...
three:					{
		...
		System.out.println();
		//Иллюстрация влияния позиции метки на поведение цилков
		int x=0, y=0;
...
outerloop1:		for(i=2; i<=100; i++){
				System.out.print("\nДелители " + i + ": ");
				for(int j=2, c=0; j<i; j++){
					if ((i%j) == 0)
						System.out.print(j + " ");
						c++;	
					if(c>5) continue outerloop1;
				}
			}
			System.out.println();
	}
